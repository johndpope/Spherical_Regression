...
class Ordered_EasyDict(object):
    ...
    # def next(self):  # no 'next' for dictionary type
    #     return self._odict.next()
    #     # if self.i < len(self):  # (self._ordered_keys):
    #     #     k = self._ordered_keys[self.i]
    #     #     self.__dict__['i'] += 1        # avoid to call __setattr__ here
    #     #     #print self.i, ' ', k, ' '
    #     #     return k, self._odict[k]
    #     # else:
    #     #     raise StopIteration()
...
def test_case4():
    fn = '/Users/shine/working/bmvc18regpose/code/trainval.pytorch.v2/binCls/fineBin+coarseBin/TRAIN.cate12aug/module.conf.yml'
    from pytorch_util.libtrain.yaml_netconf import parse_yaml, import_module_v2
    yml = parse_yaml(open(fn).read())
    yml = Ordered_EasyDict(yml)
    # print yml.keys()
    print (yml.import_module.dataset.pascal3d_gtonly['import']) #['from'] # Note: 'from' 'import' are python keywords that can't be called by oedict.from

    print ('import' in yml.import_module.dataset.pascal3d_gtonly)
