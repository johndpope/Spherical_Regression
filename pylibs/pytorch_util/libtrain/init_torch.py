...
cfg.caffemodel.alexnet = edict(
    proto = os.path.join( env.Home, 'working/Caffes/models/bvlc_alexnet/deploy.prototxt'),
    model = os.path.join( env.Home, 'working/Caffes/models/bvlc_alexnet/bvlc_alexnet.caffemodel'),
    pkl   = os.path.join( env.Home, 'working/Caffes/models/bvlc_alexnet/bvlc_alexnet.pkl'),
    input_size = (227,227),
)
cfg.caffemodel.caffenet = edict(
    proto = os.path.join( env.Home, 'working/Caffes/models/bvlc_reference_caffenet/deploy.prototxt'),
    model = os.path.join( env.Home, 'working/Caffes/models/bvlc_reference_caffenet/bvlc_reference_caffenet.caffemodel'),
    pkl   = os.path.join( env.Home, 'working/Caffes/models/bvlc_reference_caffenet/bvlc_reference_caffenet.pkl'),
    input_size = (227,227),
)
cfg.caffemodel.vgg16 = edict(
    proto = os.path.join( env.Home, 'working/Caffes/models/vgg_net/VGG_ILSVRC_16_layers_deploy.prototxt'),
    model = os.path.join( env.Home, 'working/Caffes/models/vgg_net/VGG_ILSVRC_16_layers.caffemodel'),
    pkl   = os.path.join( env.Home, 'working/Caffes/models/vgg_net/VGG_ILSVRC_16_layers.pkl'),
    input_size = (224,224),
)
cfg.caffemodel.vgg19 = edict(
    proto = os.path.join( env.Home, 'working/Caffes/models/vgg_net/VGG_ILSVRC_19_layers_deploy.prototxt'),
    model = os.path.join( env.Home, 'working/Caffes/models/vgg_net/VGG_ILSVRC_19_layers.caffemodel'),
    pkl   = os.path.join( env.Home, 'working/Caffes/models/vgg_net/VGG_ILSVRC_19_layers.pkl'),
    input_size = (224,224),
)
cfg.caffemodel.GoogLeNet = edict(
    proto = os.path.join( env.Home, 'working/Caffes/models/bvlc_googlenet/deploy.prototxt'),
    model = os.path.join( env.Home, 'working/Caffes/models/bvlc_googlenet/bvlc_googlenet.caffemodel'),
    input_size = (224,224),
)
cfg.caffemodel.vggm = edict(
    proto = os.path.join( env.Home, 'working/Caffes/models/vgg_net/VGG_CNN_M_deploy.prototxt'),
    model = os.path.join( env.Home, 'working/Caffes/models/vgg_net/VGG_CNN_M.caffemodel'),
    pkl   = os.path.join( env.Home, 'working/Caffes/models/vgg_net/VGG_CNN_M.pkl'),
    input_size = (224,224),
)
#--------------------------------------------------------- [torchmodel]
...
'''
def printNetShape(net):
    print '=============================='
    def data_summary(data_shape):
        if len(data_shape)==0:
            return '()', 0.
        mem = reduce(lambda x,y:x*y, data_shape)*4./1024/1024 # np.cumprod(np.array(data_shape,dtype=np.float32))*4/1024/1024
        return "%-20s  %8.1f M " % (str(data_shape), mem), mem
    #
    # Display blob shape
    print '------------------------------[blob shape]'
    blob_mem = 0.
    for layer_name, blob in net.blobs.iteritems():
        summary_str, _mem = data_summary(blob.data.shape)
        print "%-20s   %s  " % (layer_name,  summary_str)
        blob_mem += _mem
    print '------------------------------[total memory] %.1f G' % (blob_mem/1024)

    # Display param shape
    print '------------------------------[param shape]'
    param_mem = 0
    for layer_name, param in net.params.iteritems():    # Most param blob has w and b, but for PReLU there's only w.
        # print "%-20%   %-20s  %.1f M " % (layer_name, str(blob.data.shape), np.cumprod(np.array(blob.data.shape))/1024/1024 )
        summary_strs, layer_mem = [], 0
        for i in range(len(param)):
            summary_str, _mem = data_summary(param[i].data.shape)
            summary_strs.append(summary_str)
            layer_mem += _mem
        print "%-20s   %s  " % (layer_name,  "   ".join( summary_strs ) )
        param_mem += layer_mem
    print '------------------------------[total memory] %.1f G' % (param_mem/1024)
        # print layer_name + '\t' + " ".join( [str(param[i].data.shape) for i in range(len(param))] )
    print '=============================='

def statsNetParams(net):
    print '------------------------------[param min~max statistic]'
    print '\t%-10s\t%-10s\t%-10s\t%-10s\t%-10s\t%-10s\t%-10s\t%-10s\t' % ('min_w','max_w','mean_w','std_w','min_b','max_b','mean_b','std_b')
    for layer_name, param in net.params.iteritems():    # Most param blob has w and b, but for PReLU there's only w.
        print layer_name + '\t',
        for i in range(len(param)):
            print '%-10s\t%-10s\t%-10s\t%-10s\t' % (np.amin(param[i].data), np.amax(param[i].data), np.mean(param[i].data), np.std(param[i].data)),  # amax: Maximum of the flattened array
        print ''
        # " ".join( [str(param[i].data.shape) for i in range(len(param))] )
    print '=============================='
'''
...
def list_models(snapshots_dir, marker='iter'):
    ...
    # net_names_state = set()
    # iter_num_state  = []
    # state_files = [x for x in _files if x.endswith('.solverstate')]
    # for name in state_files:
    #     gs = re.search(reg_state, name).groups()
    #     assert len(gs)==2, '[Exception] in matching solverstate file: "%s"' % name
    #     net_name, iter_num = gs[0], int(gs[1])
    #     net_names_state.add(net_name)
    #     iter_num_state.append(iter_num)
    # assert len(net_names_state)==1, "None or Multiple net models in this dir: %s " % net_names_state
    # assert len(set(iter_num_state))==len(iter_num_state)
    # iter_num_state.sort()

    # assert iter_num_model==iter_num_state, ".caffemodel and .solverstate files aren't in pairs: \n%s\n%s\n" % (iter_num_model, iter_num_state)
    ...
    if type=='KEEP_LATEST':
        ...
        if len(saved_nums)==1:  # only has latest model.
            ...
            for it in saved_nums[:-1]:
                ...
                # solverstate_path = model_prefix+'.solverstate'
                # assert os.path.exists(caffemodel_path) and os.path.exists(solverstate_path)
                # os.system('rm -f %s %s' % (caffemodel_path, solverstate_path))
...
def _copy_weights_from_caffemodel( own_state, pretrained_type='alexnet', ignore_missing_dst=False,
    ...
    if isinstance(pretrained_type, tuple):
        ...
        import pickle
        pretrained_weights = pickle.load( open(cfg.caffemodel[pretrained_type].pkl, 'rb'), encoding='latin1')
...
def _copy_weights_from_torchmodel(own_state, pretrained_type='alexnet', strict=True, src2dsts=None):
    ...
    print ("[Info] Copy from  %s " % pretrained_type)
    print ("-----------------------")
    # src_state = model_zoo.load_url(model_urls[pretrained_type]) #['alexnet'])
...
def init_weights_by_filling(nn_module_or_seq, gaussian_std=0.01, kaiming_normal=True):
...
''' [Code snippet] from  https://github.com/pytorch/vision/blob/master/torchvision/models/vgg.py:
