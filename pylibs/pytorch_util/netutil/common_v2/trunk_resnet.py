...
import torch
from pytorch_util.libtrain import init_weights_by_filling
...
def _nn_xNorm(num_channels, xNorm='BN', **kwargs):  # kwargs:  BN: {affine=False},   LN: {elementwise_affine=True}
    ...
    # elif xNorm=='CN':
    #     return ColumnNorm(num_channels)
    # elif xNorm=='BN_3rd':
    #     return BatchNorm_3rd(num_channels)
    # elif xNorm=='GN_3rd':
    #     return GroupNorm_3rd(num_channels)
    # elif xNorm=='LN_3rd':
    #     return LayerNorm_3rd(num_channels)
    # elif xNorm=='Bl2N_3rd':
    #     return BatchL2Norm_3rd(num_channels, **kwargs)
...
class _ResNet_Trunk(nn.Module):
    ...
    def __init__(self, res_type='resnet101', init_weights=True,
        ...
        print ('[_ResNet_Trunk] init_weights:', init_weights)
        if init_weights==True:  # for legacy
            self.init_weights(pretrained='caffemodel')
            # self.init_weights(pretrained='caffemodel' if (self.net_arch in ['resnet101']) else None)
    ...
    def init_weights(self, pretrained='caffemodel'):
        ...
        if pretrained is None:
            ...
            model_path = env.Home+'/WorkGallery/pytools/pytorch_util/pretrained_model.cache/{}-caffe.pth'.format(self.net_arch)
            ...
            print(
            ''' [Warning] https://github.com/jwyang/faster-rcnn.pytorch
